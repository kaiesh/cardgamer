<?php
/**
 * Card Table - Shared deploy/sync helpers
 *
 ...
 *
 * 1. Writes the content to /tmp/ct-deploy-<random>.tmp as the deploy user (no sudo).
 ...
 * visible with incorrect perms — critical for files containing secrets.
 */
function sshWriteFile(
    ...
    $tmpPath = '/tmp/ct-deploy-' . bin2hex(random_bytes(6)) . '.tmp';
    ...
    $heredoc = "cat > " . escapeshellarg($tmpPath) . " << 'CTDEPLOYEOF'\n{$content}\nCTDEPLOYEOF";
